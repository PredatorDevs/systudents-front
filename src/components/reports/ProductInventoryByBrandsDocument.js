...
class ProductInventoryByBrandsDocument extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
  ...
  render() {
    ...
    return (
      <PageLetterWrapper>
        <p className='header'>Reporte de Inventario por Marcas</p>
        ...
        {/* <div className="divider" /> */}
        {
          (brandsData || []).map((element, index) => (
            <Row gutter={[0, 0]} key={index} style={{ }}>
              ...
              {
                (productStockData.filter((x) => x.productBrandId === element.id) || []).map((subelement, subindex) => (
                  <>
                    <Col span={2}>
...
export default ProductInventoryByBrandsDocument;