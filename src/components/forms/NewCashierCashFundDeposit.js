...
function NewCashierCashFundDeposit(props) {
  ...
  async function formAction() {
    if (validateData()) {
      setFetching(true);
      try {
        const movementRes = await cashiersServices.newCashFundDeposit(
          cashierId,
          formMovementAmount,
          formComments,
          formPINCode
  ...
  return (
    <Modal
      ...
      bodyStyle={{ backgroundColor: '#EEF7FF' }}
      maskClosable={false}
      open={open}
      footer={null}
    >
      {
        <>
          <p style={{ fontSize: 14, margin: 0, fontWeight: 600, color: '#1677ff' }}>{`Nueva Reposicion de Caja Chica`}</p>
          <div style={{ height: 20 }}/>
          <Row gutter={8} style={{ width: '100%' }}>
            <Col span={24}>
              <p style={{ margin: '0px 0px 0px 0px' }}>Monto a reponer:</p>  
              <InputNumber
            ...
            <Col span={24}>
              ...
              <TextArea
                ...
                name={'formComments'}
                value={formComments}
                placeholder={'Reposicion inicial de caja chica'}
              />
            </Col>
            <Col span={24}>
              <div style={{ height: 5 }}/>
...
export default NewCashierCashFundDeposit;
