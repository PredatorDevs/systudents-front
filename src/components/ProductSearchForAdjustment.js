...
function ProductSearchForAdjustment(props) {
  ...
  const { open, productFilterSearch, priceScale, allowOutOfStock, onClose } = props;
  ...
  function loadProductData(filter) {
    if (validateStringData(filter)) {
      setFetching(true);
      productsServices.findByMultipleParams(getUserLocation(), filter, 1)
      .then((response) => {
    ...
    >
      {
        fetching ? (
          <Result
            status="info"
            title={<p style={{ color: '#262626' }}>Buscando producto...</p>}
          />
        ) : <>
          {
                ...
                >
                  <Row gutter={[12, 12]}>
                    ...
                    <Col span={12}>
                      ...
                      <p style={styleSheet.labelStyle}>{`${detailQuantity || 0} X ${selectedProductData.productName}`}</p>
                      <p style={styleSheet.labelStyle}>{}</p>
                    </Col>
                    {/* <Col span={12}>
...
export default ProductSearchForAdjustment;
