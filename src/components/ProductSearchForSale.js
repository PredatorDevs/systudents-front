...
import { TabsContainer } from '../styled-components/TabsContainer';
...
function ProductSearchForSale(props) {
  ...
  const { open, productFilterSearch, priceScale, allowOutOfStock, distributionType, isNoTaxableOperation, onClose } = props;
      ...
      .then((response) => {
        ...
        if (response.data.length === 1) {
          ...
          if (distributionType === 1) {
            const selProd = response.data[0];
            let prodPrice = 0;

            if (response.data[0]?.pricesData?.length !== undefined) {
              if (response.data[0]?.pricesData?.length >= priceScale) {
                prodPrice = +(response.data[0]?.pricesData[priceScale - 1]?.price || 0);
              } else {
                prodPrice = +(response.data[0]?.pricesData[response.data[0]?.pricesData?.length - 1]?.price || 0);
              }
            }
            ...
            if (!(+prodPrice >= +selProd.productTotalCost)) {
              customNot('warning', 'El precio de venta no debe ser menor al costo de compra', 'Dato no válido');
              return;
            }
            
            if (
              validateSelectedData(selProd, 'No se ha podido definir un producto para la venta')
              && validateNumberData(prodPrice, 'Debe definir un precio válido')
            ) {
              const detailToAdd = new SaleDetailModel(
                ...
                +prodPrice || 0,
                selProd.taxesData,
                selProd.productIsService
              );

              restoreState();

              onClose(detailToAdd, true, selProd.currentStock);
            }
