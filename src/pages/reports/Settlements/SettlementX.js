...
function SettlementX(props) {
  ...
  async function loadSettlementData() {
    try {
      setFetching(true);
      const res = await shiftcutsService.settlementsByLocationCashierDate(locationId, cashierId, dateFilter.format('YYYY-MM-DD'));
    ...
    <Wrapper>
      <Row gutter={[8, 8]} style={{ width: '100%' }}>
        <Col span={24}>
          <Space>
            <>
              <p style={{ margin: '0px 0px 0px 0px' }}> {`Día:`}</p>
              <DatePicker 
                locale={locale}
                allowClear={false}
                format="DD-MMMM-YYYY"
                      ...
                      <Space>
                        <Button
                          ...
                          onClick={async (e) => {
                            setFetching(true);
                            try {
                              const res = await reportsServices.shiftcutXSettlement(record.shiftcutId);

                              download(res.data, `CorteX-#${record.shiftcutNumber}${record.cashierName}${record.locationName}.pdf`.replace(/ /g,''));
                            } catch(error) {
                        ...
                        <Button
                          ...
                          onClick={async (e) => {
                            ...
                            try {
                              ...
                              download(res.data, `ResumenCorte${record.shiftcutNumber}${record.cashierName}${record.locationName}.pdf`.replace(/ /g,''));
                            } catch(error) {
