...
import RawMaterialSearchForPurchase from '../../components/RawMaterialSearchForPurchase';
...
function NewRawMaterialPurchase() {
  ...
  const [rawMaterialSearchFilter, setRawMaterialSearchFilter] = useState('');

  const [openSupplierSelector, setOpenSupplierSelector] = useState(false);
  const [openRawMaterialSearchForPurchase, setOpenRawMaterialSearchForPurchase] = useState(false);
  
  const [openForm, setOpenForm] = useState(false);
  const [openRawMaterialForm, setOpenRawMaterialForm] = useState(false);
  ...
  const [rawMaterialPurchaseNotes, setRawMaterialPurchaseNotes] = useState('');
  ...
  function restoreState() {
    ...
    setRawMaterialSearchFilter('');
    setOpenSupplierSelector(false);
    setOpenRawMaterialSearchForPurchase(false);
    setOpenForm(false);
    setOpenRawMaterialForm(false);
    ...
    setFormRawMaterialDistributionId(null);
    setRawMaterialPurchaseNotes('');
  }
  ...
  function validateData() {
    ...
    return (
      ...
      && validateSelectedData(formRawMaterialDistributionId, 'Debe seleccionar una localidad para compra')
  ...
  async function createNewProdPurchase(userPINCode) {
    setFetching(true);
    try {
      const res = await rawMaterialPurchasesServices.add(
        getUserLocation(),
        supplierSelected,
        ...
        docExpirationDays || 8,
        userPINCode,
        formRawMaterialDistributionId,
        rawMaterialPurchaseNotes || ''
      );
      ...
      const detRes = await rawMaterialPurchasesServices.details.add(bulkData);
      restoreState();
    } catch(error) {
      // console.log(error);
    }
  ...
  return (
    <Wrapper>
      <Row gutter={[8, 8]} style={{ width: '100%' }}>
        ...
        >
          ...
          <Space wrap>
            ...
            <Button
              // type='primary'
              onClick={() => setOpenRawMaterialForm(true)}
            >
              <Space>
        ...
        <Col span={6}>
          ...
          <DatePicker 
            ...
            onChange={(datetimeMoment, datetimeString) => {
              ...
              document.getElementById('g-supplier-new-rawmaterial-purchase-selector').focus();
            }}
          />
        </Col>
        <Col span={12}>
          ...
          <Select
            id={'g-supplier-new-rawmaterial-purchase-selector'}
            ...
            onChange={(value) => {
              ...
              setSupplierSearchFilter('');
              document.getElementById('newsale-rawmaterial-search-input').focus();
            }}
        ...
        <Col span={24}></Col>
        <Col span={12}>
          <p style={{ margin: '0px' }}>{'Buscar materia prima:'}</p>
          <Search
            id={'newsale-rawmaterial-search-input'}
            name={'filter'}
            value={rawMaterialSearchFilter} 
            placeholder="Código, Nombre, Cód. Barra"
            allowClear 
            style={{ width: 300 }}
            onChange={(e) => setRawMaterialSearchFilter(e.target.value)}
            onSearch={(e) => setOpenRawMaterialSearchForPurchase(true)}
            onKeyDown={
              (e) => {
                if (e.key === 'Enter')
                  setOpenRawMaterialSearchForPurchase(true);
              }
            }
          />
        </Col>
        <Col span={12}>
          ...
          <TextArea style={{ resize: 'none' }} rows={3} value={rawMaterialPurchaseNotes} onChange={(e) => setRawMaterialPurchaseNotes(e.target.value)}/>
        </Col>
        <Col span={24}>
          <Table 
            // bordered
                    ...
                    <div style={styleSheet.tableFooter.footerCotainer}>
                      ...
                      <div style={styleSheet.tableFooter.detailContainer}>
                        ...
                        <InputNumber 
                          prefix={'$'}
                          size='small'
                          id='g-new-rawmaterialpurchase-iva-perception'
                          style={{ width: '50%' }} 
                          placeholder={''}
                          ...
                          onKeyDown={
                            (e) => {
                              if (e.key === 'Enter')
                                document.getElementById('g-new-rawmaterialpurchase-iva-retention').focus();
                            }
                          }
                        />
                      </div>
                      <div style={styleSheet.tableFooter.detailContainer}>
                        ...
                        <InputNumber 
                          prefix={'$'}
                          size='small'
                          id='g-new-rawmaterialpurchase-iva-retention'
                          style={{ width: '50%' }} 
                          placeholder={''}
                          min={0}
      ...
      <RawMaterialSearchForPurchase 
        open={openRawMaterialSearchForPurchase} 
        rawMaterialFilterSearch={rawMaterialSearchFilter}
        onClose={(purchaseDetailToPush, executePush) => { 
          ...
          document.getElementById('newsale-rawmaterial-search-input').focus();
          document.getElementById('newsale-rawmaterial-search-input').select();
        }}
      />
      <SupplierForm 
        open={openForm} 
